...
def save_to_google_sheet(record):
    record["Spec_Type"] = ", ".join(record.get("Spec_Type", {}).keys())
    record["Update_Time"] = datetime.datetime.now().strftime("%Y/%m/%d %H:%M")
    row = [record.get(col, "") for col in SHEET_HEADERS]
    sheet.append_row(row)

# ========== 匯出 Excel（使用範本） ==========
def export_to_excel_with_template(record):
    wb = openpyxl.load_workbook("Kipo_開案申請表 - 0918.xlsx")
    ws = wb.active
...
def render_customer_info():
    st.subheader(f"北辦業務：{st.session_state.get('user','')}")
    st.header("A. 客戶資訊")
...
def form_page():
    ...
    if st.button("✅ 完成"):
        ...
        elif not all(project_info.values()):
...
def preview_page():
    ...
    st.write(f"**ODM客戶(RD)：** {record.get('ODM_Customers','')}")
    st.write(f"**品牌客戶(RD)：** {record.get('Brand_Customers','')}")
    st.write(f"**申請目的：** {record.get('Application_Purpose','')}")
    st.write(f"**客戶專案名稱：** {record.get('Project_Name','')}")
    st.write(f"**客戶提案日期：** {record.get('Proposal_Date','')}")

    st.subheader("B. 開案資訊")
    st.write(f"**產品應用：** {record.get('Product_Application','')}")
    st.write(f"**散熱方式：** {record.get('Cooling_Solution','')}")
    st.write(f"**交貨地點：** {record.get('Delivery_Location','')}")
    st.write(f"**樣品需求日期：** {record.get('Sample_Date','')}")
    st.write(f"**樣品需求數量：** {record.get('Sample_Qty','')}")
    st.write(f"**需求量(預估數量/總年數)：** {record.get('Demand_Qty','')}")
    st.write("**Schedule**")
    st.write(f"SI：{record.get('SI','')}   PV：{record.get('PV','')}   MV：{record.get('MV','')}   MP：{record.get('MP','')}")
    ...
    if col2.button("💾 確認送出"):
        ...
        excel_data = export_to_excel_with_template(record)
        st.download_button(
            label="⬇️ 下載Excel檔案",
            data=excel_data,
